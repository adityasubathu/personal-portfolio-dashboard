...
- Market Sentiment charts in a side-by-side row (RSI/MACD, ATR/RV) now always show the full selected time range instead of only the bars that fit at the default bar spacing in a half-width column. (2026-09-21-19-59-32 · pending)
- Removed the cross-chart price-scale-width sync ("x-axis alignment") on the Market Sentiment page — it existed to keep every chart's x-axis pixel-aligned when they were all full-width and stacked, which no longer applies now that charts sit in independent side-by-side columns. (2026-09-21-19-59-32 · pending)
...
- Fixed Market Sentiment charts rapidly resizing horizontally — the shared price-scale-width sync no longer re-measures on every parent re-render, only when data actually changes. (2026-09-21-19-48-19 · 253cd49)
...
- Unified operations and settings route shells, controls, and destructive triggers. (2026-09-21-12-50-21 · d78046b)
- Standardized dense table typography and spacing across portfolio pages and Kite. (2026-09-21-12-56-27 · dd3807d)
- Documented the desktop visual grammar and verified route-width constraints. (2026-09-21-12-58-07 · pending)
...
- Removed the mobile-only render paths: the Dashboard holdings accordion view, the Trades order cards, and the two "scroll horizontally" hints on Breakdown. The `useMediaQuery` and `useColorScheme` hooks went with them. (2026-09-20-17-18-05 · e7caaac)
- Collapsed `sm:`/`md:` responsive classes to their desktop values across Dashboard, Breakdown, CapitalGains, FundBreakdown, NavHistory and AppLayout. Desktop rendering is unchanged. (2026-09-20-17-18-05 · e7caaac)
- Hoisted the two duplicated `useDebouncedValue` copies into `hooks/useDebouncedValue.ts` and the two `InfoPopover` variants into `components/InfoPopover.tsx`. (2026-09-20-17-18-05 · e7caaac)
- Dropped the dead `--chart-1..5` theme tokens and the vestigial `mantine` CSS layer left over from the Mantine build. (2026-09-20-17-18-05 · e7caaac)
...
- **Orphaned endpoints removed**, each reachable only through a React Query hook no page ever called: `GET /api/v1/mf-breakdown/stock-holdings`, `GET /direct-trades`, `GET /allocation-targets`, `GET /asset-class-targets` (the `POST` twins of the last two are still in use), `POST /api/v1/mf/fetch-nav-by-isin`, and `GET /api/v1/kite/config`. That also retired `get_stock_holdings_table` (85 lines, `allocation.py`), `get_direct_trade_breakdown` (117 lines, `composition.py`) and `fetch_nav_by_isin` (65 lines, `mfapi_nav.py`); the Breakdown page lost its Direct Trades tab a while back but the query behind it stayed.
- **`GET /api/v1/kite/auth/url` removed** — it recomputed `kite_client.login_url(...)`, which `GET /status` already returns as `login_url` and which nothing consumed. `Kite.tsx` now points the login button straight at `status.login_url` instead of `preventDefault`-ing and re-fetching the same string.
- **`Violation` corrected rather than deleted** — `app/schemas/trades.py` declared `{symbol, isin, kind, detail}` while `holdings_engine` emits `{instrument_id, tradingsymbol, isin, instrument_type, total_buy, total_sell, net}`, and `ImportResponse.violations` was typed `list[dict]`, so the model was never exercised. Given the real shape and wired into `ImportResponse`; `frontend/src/types/trades.ts` mirrors it and the Import page now renders the actual numbers — it had been printing `undefined: <isin> — undefined` for every violation.
- **`frontend` build unblocked** — `tsc -b` was failing on five unused locals plus `chart.unsubscribeCrosshairMove(sub)`, where `sub` held the `void` return of `subscribeCrosshairMove`. The crosshair callback is now a named `onCrosshairMove` handler passed to both subscribe and unsubscribe, so the cleanup actually detaches it.
- **Other dead code** — `PctText` and `gainColorRb` (frontend, no callers); `MfSyncResult`/`FetchNavResult` schemas and the `KiteConfig`, `StockHolding`, `DirectTradeBreakdown`, `FetchNavResult` TS types; `Settings.kite_api_key`/`kite_api_secret` (credentials live in the `kite_config` table) and their `.env.example` keys; `beautifulsoup4` and `@mantine/dates`, neither imported anywhere; ten unused imports across the backend. `allocation.py`'s three lazy `get_manual_assets_summary` imports (one of them unused) collapsed into one module-level import — there is no import cycle to dodge.
- `README.md` — dropped the removed endpoints from the API tables, fixed the `schemas/` and `services/` trees (both still listed files from the `mf_breakdown` split and a `fetch_nifty50_ohlc.py` that no longer exists), and added `usePrivacy.tsx`.
