...
export function heatmapBg(
  ...
  if (value > 0 && max != null && max > 0) {
    ...
    return `color-mix(in oklab, ${color} ${Math.round(10 + intensity * 14)}%, var(--surface-panel))`
  }
  if (value < 0 && min != null && min < 0) {
    const intensity = Math.min(Math.abs(value) / Math.abs(min), 1)
    return `color-mix(in oklab, var(--negative) ${Math.round(10 + intensity * 14)}%, var(--surface-panel))`
  }
  return undefined
}
